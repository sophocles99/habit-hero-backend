...
// Adjust settings to create different amounts of data
// The full number of users is generated, each user has up to the specified
// number of habits, and each habit has up to the specified number of actions
const testSettings = { users: 3, habits: 5, actions: 30 };
const devSettings = { users: 10, habits: 5, actions: 30 };
...
function generateData({ users, habits, actions }) {
  const data = [];
  for (let i = 0; i < users; i++) {
    ...
    const numberOfHabits = Math.floor(Math.random() * habits + 1);
    for (let j = 0; j < numberOfHabits; j++) {
      ...
      const numberOfActions = Math.floor(Math.random() * actions + 1);
      const daysAgo = [];
